...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    cp -Rf ../../cryptography/org1.example.com organizations/peerOrganizations/
    ...
    cp -Rf ../../cryptography/org2.example.com organizations/peerOrganizations/
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    cp -Rf ../../cryptography/ca-config-org1.example.com/* organizations/fabric-ca/org1/
    cp -Rf ../../cryptography/ca-config-org2.example.com/* organizations/fabric-ca/org2/
...
function deployCC() {
  ##infoln "Deployyy: $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE"
